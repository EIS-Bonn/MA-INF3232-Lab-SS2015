...
if(isset($_GET['action']))
{
	switch ($_GET['action']){
		...
		case 'word_ins' :
			$word=$_GET['word'];
			$sense=$_GET['sent'];
		    word_insert($word,$sense);
			
			break;			
	}
}

function word_insert($word,$sent)
{

$link=dbconnect();
if ($word)
{	
	$query="INSERT INTO `adjectives` ( `name`, `sense`) VALUES ( '".$word."', '".$sent."')";
	$result=mysql_query($query);
}
if ($result)
	echo "<div id='w1results'>Add new word </div>". $word;
else 
	echo "<div id='w1results'>Try again </div>". $word;
mysql_close();
}
...
$link=dbconnect();
    $query="SELECT * FROM  `adjectives` WHERE name like '".$_GET['word']."%';";
	$result=mysql_query($query);
	$pos='positive';
	$neg='negative';
	$neu='neutral';
...
function insert_opinion_news()
{
	...
	$link=dbconnect();
     $query="INSERT INTO `news_opi` (`idn`,`opinion`) VALUES ( '".$idn."','".$sent."'); ";
	$result=mysql_query($query);
	if ($result)
	{
		echo "<div class='alert alert-dismissable alert-success'>Your opinion add successfully</div>";
...
function findopinion($pos,$neg,$neu,$id)
{
		if ($pos>$neg )
		{
			...
			$result=mysql_query($query);
		}else if($neg>$neu && $pos!=$neg)
		{
			$query="UPDATE  `adjectives` SET  `sense` =  'negative' WHERE  `adjectives`.`id` =".$id.";";
			$result=mysql_query($query);
		}else
		{
			$query="UPDATE  `adjectives` SET  `sense` =  'neutral' WHERE  `adjectives`.`id` =".$id.";";
			$result=mysql_query($query);
		}
...
function news_sense()
{
    ...
    $query="SELECT * FROM news_sense where txt not like '' and id  in (select idn from news_opi) and sentiment not like sentiment2 order by id ASC LIMIT 0 , 1000 ;";
	$result=mysql_query($query);
	while ($row = mysql_fetch_assoc($result)) {
		echo"<div id='news".$row['id']."' class='news'>";
				/*switch ($row['sentiment']){
					case "positive":
						echo "<div id='title-news'	class='title-news-pos' onclick='show_news_txt(".$row['id'].")'>".$row['id']."  ".$row['title']."  sent: ".$row['sentiment']."  sent2: ".$row['sentiment2']."<span class='caret'></span></div>";
						break;
					case "negative":
						echo "<div id='title-news'	class='title-news-neg' onclick='show_news_txt(".$row['id'].")'>".$row['id']."  ".$row['title']."  sent: ".$row['sentiment']."  sent2: ".$row['sentiment2']."<span class='caret'></span></div>";
						break;
					case "neutral":*/
						echo "<div id='title-news'	class='title-news-neu' onclick='show_news_txt(".$row['id'].")'>".$row['id']."  ".$row['title']."  sent: ".$row['sentiment']."  sent2: ".$row['sentiment2']."<span class='caret'></span></div>";
						//break;
				//}
				echo"<div id='news-text".$row['id']."' style='display:none;'	class='news-text'>".$row['txt']."</div>
					<div class='vote-news'>					
						<button type='button' class='btn btn-success btn-sm' onclick='news_opinion(".$row['id'].",1);'>مثبت</button> 
						<button type='button' class='btn btn-primary btn-sm' onclick='news_opinion(".$row['id'].",2);'>خنثی</button>
						<button type='button' class='btn btn-danger btn-sm' onclick='news_opinion(".$row['id'].",3);' >منفی</button>
					</div>
			</div>";
	}
	echo "<p ><a href='/news.php' class='btn btn-danger btn-sm' >Next news</a> </p>";
}
