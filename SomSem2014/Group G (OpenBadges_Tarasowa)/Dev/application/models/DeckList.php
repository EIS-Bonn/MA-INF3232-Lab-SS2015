...
class DeckList extends Model {
	...
	function getAllDecks($limit = 30,$exceptFeatured=0){
		...
		// query here
		$res = $this->dbQuery( 'SELECT * FROM deck WHERE translated_from IS NULL');
		$decks = array();
		foreach($res as $index => $deck){
			$deck1 = new Deck();
			$deck1->id = $deck['id'];
			$decks[] = $deck1;
		}		
		...
		// query here
		$res = $this->dbQuery( 'SELECT * FROM deck WHERE translated_from IS NOT NULL');
		$decks = array();
		foreach($res as $index => $deck){
			$deck1 = new Deck();
			$deck1->id = $deck['id'];
                ...
                if ($this->own){//only own decks
                    if (!$language){
                        if (strlen($keywords)>2) {
                            if ($tag=='all') {				
                                ...
                                $query =  'SELECT * FROM deck_revision WHERE deck_revision.user_id=:user_id AND id IN
                                                (SELECT item_id FROM tag WHERE ' . $subquery . ') '.$cond1.'GROUP BY `deck_id` ORDER BY ' . $order_db ;	
                                }
                        }
                        ...
                        if (strlen($keywords)>2) {
                            if ($tag=='all') {				
                                ...
                                if ($tag=='all') {				
                                        $query =  'SELECT * FROM deck_revision JOIN deck ON deck_revision.deck_id=deck.id' .$cond11.' AND deck_revision.user_id=:user_id AND deck.language=:language GROUP BY `deck_id` ORDER BY  ' . $order_db   ;
                                } else {
                                    $tag_array = explode(',',$tag);
                                $subquery = 'tag = "' . $tag_array[0] . '"';
                                if (count($tag_array) > 1){
                    ...
                    if (!$language){
                        ...
                        if (strlen($keywords)>2) {
                                ...
                                if ($tag=='all') {				
                                        $query =  'SELECT * FROM deck_revision JOIN deck ON deck_revision.deck_id=deck.id WHERE deck.language=:language' .$cond1.' GROUP BY `deck_id` ORDER BY  ' . $order_db   ;
                                } else {
                                    $tag_array = explode(',',$tag);
                                    $subquery = 'tag = "' . $tag_array[0] . '"';
                                    if (count($tag_array) > 1){
                                        ...
                                        $query =  'SELECT * FROM deck_revision JOIN deck ON deck_revision.deck_id=deck.id WHERE deck_revision.id IN
                                                (SELECT item_id FROM tag WHERE ' . $subquery . ') '.$cond1.' AND deck.language=:language GROUP BY `deck_id` ORDER BY ' . $order_db ;	
                                }
                        }
